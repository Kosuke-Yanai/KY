...
class Scraper:
    def __init__(self, site, word):
        self.site = site
        self.word = word
        
    def scrape(self): 
        # URLエンコーディング
        #s_quote = urllib.parse.quote(self.word)
        #self.site = self.site + "?q=" +s_quote + "&hl=ja&gl=JP&ceid=JP%3Aja"
        #パラメータ生成
        params = {'hl':'ja', 'gl':'JP', 'ceid':'JP:ja', 'q':self.word}
        # urlを設定してリクエストを送る
        res = requests.get(self.site, params=params)
        ...
        for i, h3_entry in enumerate(h3_blocks):
            with open(self.word + ".txt", mode='a') as f:
